...
def numeros(num):
    if len(pantalla.get()) > 0:
        pantalla.insert(len(pantalla.get()),num)
    else:
        pantalla.insert(0,num)
...
def calcular():
    ...
    n1 = txt[0:txt.index(oper)]
    n2 = txt[txt.index(oper)+1:: ]
    if oper == "+":
        pantalla.insert(0,float(n1)+float(n2))
    elif oper == "-":
        pantalla.insert(0,float(n1)-float(n2))
    elif oper == "*":
        pantalla.insert(0,float(n1)*float(n2))
...
boton_1 = Button(root, text="1",width=9, height=3, bg="white", fg="red", borderwidth=0, cursor="hand2", command=lambda: numeros(1)).grid(row=1, column=0, padx=1, pady=1)
...
boton_igual = Button(root, text="=",width=20, height=3, bg="red", fg="white", borderwidth=0, cursor=" hand2", command= lambda:calcular()).grid(row=4, column=0, columnspan=2, padx=1, pady=1)  #duda con ese columnspan
